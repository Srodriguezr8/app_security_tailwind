...
def get_group_menus(request):
    ...
    try:
        ...
        menus = (
            Menu.objects.filter(
                modules__group_permissions__group=group, modules__is_active=True
            )
            .distinct()
            .order_by("order", "name")
        )
        ...
        print(": ", request.session["selected_group_id"])
        ...
        return JsonResponse({"success": True, "menus": menu_list})
...
def _serialize_menus_with_modules(menus, group):
    ...
    for menu in menus:
        ...
        modules = (
            menu.modules.filter(group_permissions__group=group, is_active=True)
            .distinct()
            .order_by("order", "name")
        )
